...
def setup_dirs():
    try:
        ...
        if any(x in service_name for x in "~!@#$%^&*()+{}|[]\\:\"'<>?,/") == True:
            print("Please don't use the following characters for your project name:\n~!@#$%^&*()_+{}|[]\\:\"'<>?,/")
            raise ValueError
        print("Setting up service: {:s}".format(service_name))
    except Exception as err:
        print("USAGE: setupvenv <product name> <target location>")
        sys.exit(1)
    basedir = os.path.join(os.getcwd(), service_name)
    try:
        os.makedirs(basedir)
    except:
        print("Directory exists. Using it")
